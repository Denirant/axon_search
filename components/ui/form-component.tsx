...
import { ScanEye, X, Telescope, Settings } from 'lucide-react';
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
const models = [
    ...
    },
    {
        ...
        icon: '/openai.svg',
        iconClass: '!text-neutral-300',
        description: 'Smartest Nano model',
        color: 'indigo',
        vision: false,
        experimental: false,
        category: 'Stable',
    },
    {
        ...
        icon: '/openai.svg',
        iconClass: '!text-neutral-300',
        description: 'Fasterst Mini model',
        color: 'green',
        vision: true,
        experimental: false,
        category: 'Stable',
    ...
    // 'scira-gemma3-12b': openrouter('google/gemma-3-12b-it:free'),
    {
        value: 'scira-4o-mini',
        label: 'GPT 4o Mini',
        icon: '/openai.svg',
...
// 'scira-default': openai('gpt-4.1-nano-2025-04-14', {
//     parallelToolCalls: false,
//     structuredOutputs: true,
// }),
// 'scira-qwen': wrapLanguageModel({
//     model: deepinfra('Qwen/Qwen2.5-72B-Instruct'),
//     middleware,
// }),
// 'scira-4o-mini': openai('gpt-4o-mini-2024-07-18', {
//     structuredOutputs: true,
// }),
...
const getColorClasses = (color: string, isSelected: boolean = false) => {
    ...
    switch (color) {
        ...
        case 'green':
            return isSelected
                ? `${baseClasses} ${selectedClasses} !bg-[#4bb153] dark:!bg-[#4bb153] !text-white hover:!bg-[#3a9a43] dark:hover:!bg-[#3a9a43] !border-[#4bb153] dark:!border-[#4bb153]`
                : `${baseClasses} !text-[#4bb153] dark:!text-[#5dc866] hover:!bg-[#4bb153] hover:!text-white dark:hover:!bg-[#4bb153] dark:hover:!text-white`;
        case 'indigo':
            return isSelected
...
const ModelSwitcher: React.FC<ModelSwitcherProps> = ({
    ...
    return (
        <DropdownMenu onOpenChange={setIsOpen} modal={false} open={isOpen && !isProcessing}>
            ...
            <DropdownMenuContent
                className="w-[200px] p-1 !font-sans rounded-2xl bg-white dark:bg-neutral-900 !mt-1.5 !z-[52] shadow-lg border border-neutral-200 dark:border-neutral-800 max-h-[290px] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 scrollbar-track-transparent"
                align="start"
                side="bottom"
                sideOffset={8}
                forceMount
                        ...
                        <div className="space-y-0.5">
                            {categoryModels.map((model) => (
                                <DropdownMenuItem
                                    ...
                                    className={cn(
                                        'flex items-center gap-1.5 px-1.5 py-1 rounded-xl text-xs min-h-[36px] cursor-pointer',
                                    ...
                                    <div
                                        className={cn(
                                            ...
                                            selectedModel === model.value ||
                                                model.value === 'scira-default' ||
                                                model.value === 'scira-4o-mini' ||
                                                model.value === 'scira-4.1-mini'
...
interface FormComponentProps {
    ...
    handleSubmit: (e: React.FormEvent<HTMLFormElement>, messageContent: string, attachments?: Attachment[]) => Promise<void>;
...
const SwitchNotification: React.FC<SwitchNotificationProps> = ({
    ...
    const getModelBgClass = (color: string) => {
        switch (color) {
            ...
            case 'green':
                return 'bg-[#4bb153] dark:bg-[#4bb153] border-[#4bb153] dark:border-[#4bb153]';
            case 'indigo':
    ...
    const useModelColor = notificationType === 'model' && modelColor !== 'default' || title.toLowerCase().includes('deep research');

    const bgColorClass = (title.toLowerCase().includes('deep research')) ? getModelBgClass('gray')
        : useModelColor ? getModelBgClass(modelColor)
        ...
        <AnimatePresence>
            {isVisible && (
                ...
                >
                    <div
                        className={cn(
                            'flex items-center gap-2 py-2 px-3 sm:py-2.5 sm:px-3.5 rounded-t-2xl border shadow-sm backdrop-blur-sm',
                            bgColorClass,
...
const ToolbarButton = ({ group, isSelected, onClick }: ToolbarButtonProps) => {
    ...
    const commonClassNames = cn(
        ...
        isSelected
            ? 'bg-neutral-500 dark:bg-neutral-600 text-white dark:text-neutral-200'
            : 'text-neutral-600 dark:text-neutral-300 hover:bg-neutral-200 dark:hover:bg-neutral-800/80',
    );
...
const FormComponent: React.FC<FormComponentProps> = ({
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            // Замените прямой вызов handleSubmit на новый метод
            await handleSubmit(e, input, attachments);
    ...
    return (
        <div className="flex flex-col w-full">
            <div
                className={cn(
                    'relative w-full flex flex-col gap-1 rounded-3xl transition-all duration-300 !font-sans',
                    hasInteracted ? 'z-[51]' : '',
                ...
                <div className="relative">
                    ...
                    <div className="relative rounded-3xl bg-neutral-100 dark:bg-neutral-900">
                        <Textarea
                            ...
                            onBlur={handleBlur}
                            className={cn(
                                'w-full rounded-3xl resize-none',
                        ...
                        >
                            ...
                            >
                                ...
                                {selectedGroup == 'extreme' && (
                                    <div className='flex items-center justify-center p-0.5 border border-neutral-200 bg-white shadow-sm rounded-full'>
                                        <div className='w-[32px] h-[32px] bg-neutral-700 flex items-center justify-center rounded-full'>
                                            <Settings className='w-4 h-4 text-gray-50'/>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    onClick={(e) => {
                                        ...
                                        const newModeText =
                                            selectedGroup === 'extreme'
                                                ...
                                                : 'Switched to Deep Research';
                                        const description =
                                            selectedGroup === 'extreme'
                                                ? 'Standard web search mode is now active'
                                                : 'Enhanced deep research is now active';

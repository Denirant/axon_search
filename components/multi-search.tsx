        ...
        <div className="w-full space-y-3">
            <Accordion type="single" collapsible defaultValue="search" className="w-full">
                <AccordionItem value="search" className="border-none">
                    <AccordionTrigger className="py-3 px-4 bg-white dark:bg-neutral-900 rounded-3xl hover:no-underline border border-neutral-200 dark:border-neutral-800 data-[state=open]:rounded-b-none">
                        <div className="flex items-center justify-between w-full">
                            <div className="flex items-center gap-2">
                                <div className="p-1.5 rounded-xl bg-neutral-100 dark:bg-neutral-800">
                                    <Globe className="h-3.5 w-3.5 text-neutral-500" />
                                </div>
                                <h2 className="font-medium text-left text-sm">Результаты поиска</h2>
                            </div>
                            <div className="flex items-center gap-2">
                                <Badge
                                    variant="secondary"
                    ...
                    <AccordionContent className="pt-0 mt-0 border-0 overflow-hidden">
                        <div className="py-3 px-4 bg-white dark:bg-neutral-900 rounded-b-3xl border border-t-0 border-neutral-200 dark:border-neutral-800">
                            {/* Query badges */}
            ...
            <div className="grid gap-1.5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
                {[...Array(5)].map((_, i) => (
                    <div
                        key={i}
                        className={cn(
                            'aspect-video rounded-lg bg-neutral-200 border border-gray-100 dark:bg-neutral-800 animate-pulse',
        ...
        <div>
            <div
                className={cn(
                    ...
                    displayImages.length > 1 && 'sm:grid-rows-[1fr_1fr]',
                    // Отключаем aspect-video для ПЕРВОГО элемента при нескольких изображениях
                    displayImages.length > 1 ? '[&>*:not(:first-child)]:aspect-video' : '[&>*]:aspect-video',
            ...
            >
                {displayImages.map((image, index) => (
                    <motion.button
                        key={index}
                        className={cn(
                            'relative rounded-xl overflow-hidden group border border-gray-100 bg-gray-100',
                            'focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1',
                            'transition-all duration-200',
                            'h-full w-full',
                            // Особая обработка для первого элемента при нескольких изображениях
                            index === 0 && displayImages.length > 1 && 'sm:h-full',
                    ...
                    >
                        ...
                        />
                        {image.description && (
                            <div className="absolute bottom-1 w-3/4 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 px-2 py-1 overflow-hidden rounded-2xl bg-gray-800/80">
                                <p className="text-[12px] text-gray-100 w-full text-nowrap overflow-hidden text-ellipsis">{image.description}</p>
                            </div>
                        )}
            ...
            {isDesktop ? (
                <Dialog open={isOpen} onOpenChange={setIsOpen}>
                    <DialogContent className="max-w-4xl w-[90vw] h-[65vh] p-0 overflow-hidden border-none rounded-2xl">
                        <div className="relative w-full h-full rounded-2xl overflow-hidden bg-white dark:bg-neutral-900">
                            <div className="absolute top-0 left-0 right-0 z-50 p-3 flex justify-between items-center">
                                ...
                                <Badge
                                    variant="secondary"
                                    className="rounded-full bg-gray-100/50 px-3.5 py-1.5 dark:bg-neutral-800/90 text-black/80 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700"
                                >
                            ...
                            <div className="absolute inset-0 flex items-center justify-center px-2 mt-[55px] mb-2">
                            ...
                            <Button
                                className={cn(
                                    'absolute left-4 top-1/2 -translate-y-1/2 h-8 w-8 p-1 rounded-2xl',
                            ...
                            <Button
                                className={cn(
                                    'absolute right-4 top-1/2 -translate-y-1/2 h-8 w-8 p-1 rounded-2xl',
                                    'bg-white/90 dark:bg-neutral-800/90 text-neutral-700 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700',
                                    'hover:bg-white hover:dark:bg-neutral-800 transition-colors',
                                )}
                                onClick={() => {
                            ...
                            {images[selectedImage].description && (
                                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 rounded-xl inset-x-0 mb-5 py-2 px-4 w-[70%] bg-neutral-100/95 dark:bg-neutral-800/90 border border-neutral-200 dark:border-neutral-700">
                                    <p className="text-sm font-medium text-neutral-600 dark:text-neutral-300 mx-auto w-full text-nowrap overflow-hidden text-ellipsis">
                                        {images[selectedImage].description}
                                    </p>
                                </div>
                            )}

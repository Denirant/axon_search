...
plt.show()`
  ...
  `;
                ...
                experimental_repairToolCall: async ({
                    ...
                    if (toolCall.toolName === 'web_search' && toolCall.args) {
                        try {
                            const args = JSON.parse(toolCall.args);
                            if (args.topics && Array.isArray(args.topics)) {
                                args.topics = args.topics.map(topic => 
                                    topic === 'images' ? 'general' : topic
                                );
                                
                                // Проверяем, что все значения допустимы
                                const validTopics = ['general', 'news', 'finance'];
                                args.topics = args.topics.map(topic => 
                                    validTopics.includes(topic) ? topic : 'general'
                                );
